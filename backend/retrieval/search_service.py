...
def candidate_to_evidence(candidate: RetrievalCandidate) -> EvidenceItem:
    metadata = EvidenceMetadata(
        paper_id=str(candidate.metadata.get("paper_id", "")),
...
def render_evidence_context(evidence: List[EvidenceItem]) -> str:
    ...
    for index, item in enumerate(evidence, start=1):
        ...
        chunks.append(
            "\n".join(
                [
                    f"[{index}] paper_id={metadata.paper_id or ''} title={metadata.title or ''}",
...
def build_metrics(candidates: List[RetrievalCandidate]) -> Dict[str, float]:
    ...
    for candidate in candidates[:5]:
        source = candidate.metadata.get("paper_id") or candidate.metadata.get("source") or "unknown"
